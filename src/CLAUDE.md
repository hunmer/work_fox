...
1. **工作流可视化编辑**：基于 Vue Flow 的拖拽式 DAG 编辑器，支持节点拖入/连线/属性编辑/执行调试，含嵌入式子工作流、复合节点（loop 等）、分组节点（GroupNode）、便签节点
...
- `wsBridge`：WebSocket 桥接层，承载 workflow / execution / plugin / chat / dashboard 等 backend 通道
...
src/
  ...
  components/
    ...
    ui/                              基础 UI 组件库（shadcn-vue 风格，41 个组件目录）
...
| `ExecutionBar.vue` | 执行控制：开始/暂停/恢复/停止、执行历史、步骤详情(输入/输出/日志) |
| `NodeSidebar.vue` | 节点面板：搜索、插件配置方案管理、拖放发起 |
| `RightPanel.vue` | 右侧面板标签容器(属性/版本/操作/AI助手) |
...
**Q: UI 组件库是怎么组织的？**
A: `src/components/ui/` 采用 shadcn-vue 风格，每个组件一个独立目录，包含 Vue 组件和 `index.ts` 导出。目前有 41 个组件目录。
