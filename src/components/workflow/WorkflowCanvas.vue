...
const menuRef = ref<InstanceType<typeof CanvasContextMenu>>()
</script>

<template>
  <div class="relative h-full w-full" @contextmenu.capture="menuRef?.handleContextMenuCapture($event)">
    <CanvasContextMenu ref="menuRef">
      <VueFlow
        :id="canvas.flowId"
        ...
        @pane-contextmenu="menuRef?.onPaneContextMenu($event)"
      >
        <Background />
        <HelperLines
