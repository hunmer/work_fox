...
7. 需要在两个节点之间做数据整形、字段映射或结构转换时，优先在现有边上插入 \`run_code\` 节点。该节点的 \`code\` 中不要写 \`{{ }}\`；必须定义 \`main\` 函数，推荐写成 \`async function main({ params, context }) { ... return {...} }\`。写完 \`code\` 后，还要同步设置节点的 \`data.outputs\`，让输出字段结构与 \`main\` 的返回对象一致。
