...
7. 需要在两个节点之间做数据整形、字段映射或结构转换时，优先在现有边上插入 \`run_code\` 节点。该节点的 \`code\` 中不要写 \`{{ }}\`；必须定义 \`main\` 函数，推荐写成 \`async function main({ params, context }) { ... return {...} }\`。
8. 对数组类型参数，优先保持原有数组/对象结构；如果上游返回值本身已经是目标数组，直接把整个数组字段写成纯变量引用；只有在需要手动拼装数组项时，才在数组项的字段里写变量。
9. 如果要编辑循环体等内嵌子工作流，继续使用同一套工具，但要显式传 \`embeddedInNodeId\`，把操作范围切换到该宿主节点的 \`bodyWorkflow\`。

典型模式：
- 先 \`get_current_workflow\` 看现状。
- 再 \`search_node_usage\` 看节点的精确字段定义。
- 如果要在两个节点之间插入 JS 转换节点，使用 \`insert_node\` 插入 \`run_code\`，并把 \`code\` 写成 \`main\` 函数。
