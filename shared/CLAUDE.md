...
1. **类型安全契约**：`channel-contracts.ts` 定义所有 backend WS 通道的请求/响应类型映射（`BackendChannelMap`），含 Dashboard/Trigger/Staging/Preset 契约
...
4. **工作流类型**：`workflow-types.ts` 定义工作流核心数据模型（Workflow/Node/Edge/ExecutionLog 等），包括复合节点、嵌入式子工作流、触发器、暂存节点
...
| 文件 | 主要导出 | 消费者 |
|---|---|---|
| `channel-contracts.ts` | `BackendChannelMap`, `BackendChannel`, `ChannelRequest<C>`, `ChannelResponse<C>`, `ExecutionInputPreset`, `DashboardStatsResponse`, `DashboardExecutionsRequest/Response`, `DashboardWorkflowDetailRequest/Response` | backend/ws/*, src/lib/backend-api/*, src/lib/ws-bridge.ts |
...
| `workflow-types.ts` | `Workflow`, `WorkflowNode`, `WorkflowEdge`, `WorkflowTrigger`, `StagedNode`, `ExecutionLog`, `NodeTypeDefinition`, `CompoundNodeDefinition`, `EmbeddedWorkflow` | 全项目 |
...
- **聊天历史**: `chatHistory:listSessions/createSession/updateSession/deleteSession/listMessages/addMessage/updateMessage/deleteMessage/deleteMessages/clearMessages/importData/listAllScopeKeys`
...
- **执行输入预设**: `executionPreset:list`, `executionPreset:save`, `executionPreset:delete`
- **触发器**: `trigger:validate-cron`, `trigger:check-hook-name`
- **暂存区**: `staging:load`, `staging:save`, `staging:clear`

### 触发器类型（`WorkflowTrigger`）

```typescript
type WorkflowTrigger =
  | (WorkflowTriggerBase & { type: 'cron'; cron: string; timezone?: string })
  | (WorkflowTriggerBase & { type: 'hook'; hookName: string })
...
### 执行输入预设类型（`ExecutionInputPreset`）

```typescript
interface ExecutionInputPreset {
  id: string
  name: string
...
interface Workflow {
  // ...原有字段...
  icon?: string                    // 工作流图标（emoji）
  description?: string             // 工作流描述
  tags?: string[]                  // 标签
  triggers?: WorkflowTrigger[]     // 触发器列表
}
```
...
- **Workflow**: 完整工作流（含 nodes/edges/enabledPlugins/agentConfig/pluginConfigSchemes/layoutSnapshot/groups/triggers/icon/description/tags）
...
- **WorkflowTrigger**: 触发器（cron 或 hook，discriminated union）
- **StagedNode**: 暂存节点（用于跨工作流节点复用）
...
**Q: WorkflowTrigger 类型如何扩展？**
A: 在 `WorkflowTrigger` discriminated union 中添加新的分支（如 `{ type: 'event'; eventName: string }`），然后在 `backend/workflow/trigger-service.ts` 的 `registerTriggers` 方法中添加对应的注册逻辑。

**Q: StagedNode 和 WorkflowNode 的区别？**
A: `StagedNode` 是暂存区中的节点副本，额外包含 `sourceNodeId`（来源节点 ID）和 `stagedAt`（暂存时间），不包含 `position` 字段（粘贴时重新计算位置）。
...
shared/
  ...
  channel-contracts.ts                 WS 通道类型契约（BackendChannelMap，含 Dashboard/Trigger/Staging/Preset 契约）
  ...
  workflow-types.ts                    工作流核心数据模型（含复合节点/嵌入式工作流/触发器/暂存节点）
...
| 日期 | 操作 | 说明 |
|---|---|---|
| 2026-04-30 | 增量更新 | 新增 WorkflowTrigger（cron/hook）discriminated union 类型、StagedNode 暂存节点类型、ExecutionInputPreset 执行输入预设类型；新增 trigger:validate-cron/check-hook-name/staging:load/save/clear/executionPreset:list/save/delete 通道契约；Workflow 类型新增 icon/description/tags/triggers 字段 |
