...
- 功能：start（拓扑排序 + 顺序执行）、pause/resume/stop、switch 分支路由、复合节点展开、插件配置加载
- 支持嵌入式子工作流（如 loop 节点的 bodyWorkflow）的递归执行
...
### ChatEventSender（`chat/chat-event-sender.ts`）

- 职责：封装 ConnectionManager.sendToClient 为 9 个类型化 chat 事件发送方法
- 事件通道：chunk / toolCall / toolCallArgs / toolResult / thinking / usage / done / error / retry
- 每个 ClientId 独立发送，确保事件路由到正确的发起客户端

### ChatToolAdapter（`chat/chat-tool-adapter.ts`）

- 职责：将 WorkFox 的工具体系桥接到 Claude Agent SDK 的 MCP Server 模式
- 创建三个 MCP Server：
  - `workfox_browser`：浏览器工具（list_categories / list_tools_by_category / get_tool_detail / execute_tool）
  - `workfox_workflow`：工作流编辑工具（来自 `WORKFLOW_TOOL_DEFINITIONS`，含渲染端工具走 interaction）
  - `workfox_plugin`：插件 Agent 工具（server 插件直接执行，client 插件走 interaction）
- 工具名清洗（sanitize）确保符合 MCP 命名规则
- 支持 `canUseTool` / `resolveDisplayToolName` / `getExecutionContext`

### ChatWorkflowToolExecutor（`chat/chat-workflow-tool-executor.ts`）

- 职责：在 Chat 对话中直接操作工作流图结构
- 支持的工具：`get_workflow`、`list_node_types`、`create_node`、`update_node`、`delete_node`、`create_edge`、`delete_edge`、`insert_node`、`batch_update`、`auto_layout`、`search_nodes`、`search_node_usage`
- 支持复合节点创建（`createNodesByDefinition`）：根据 `CompoundNodeDefinition` 自动生成多个子节点和内部边
- 支持嵌入式子工作流操作（通过 `embeddedInNodeId` 参数操作节点内部的 bodyWorkflow）
- 节点数据校验：变量引用格式检查、输出字段结构校验
- 变更后通过 ConnectionManager 通知渲染进程刷新

### ClientNodeCache（`chat/client-node-cache.ts`）

- 职责：缓存来自 WS 客户端注册的插件节点定义和工具定义
- 通过 `chat:register-client-nodes` / `chat:register-client-agent-tools` 通道接收注册
- 客户端断连时自动清理
- 被三个消费者使用：
  - `ChatToolAdapter` -- 判断工具是否为 client tool（需要走 interaction）
  - `ChatWorkflowToolExecutor` -- 列出所有可用节点类型（含 client 插件节点）
  - `ChatRuntime` -- 组装完整工具链

### DashboardStatsStore（`dashboard/stats-store.ts`）

- 职责：聚合工作流和执行日志数据，提供 Dashboard 统计查询
- 方法：
  - `getStats()` -- 统计概览（工作流数/运行中/插件数/今日执行/本周执行/总执行/30 天趋势），60 秒 TTL 缓存
  - `getExecutions(req)` -- 执行历史分页（支持 today/week/all 范围过滤、状态过滤、分页）
  - `getWorkflowDetail(workflowId)` -- 工作流详情（基本信息 + 版本列表 + 执行记录）
  - `invalidateCache()` -- 手动清除缓存
- 数据来源：遍历所有工作流的 execution-log JSON 文件 + workflow-version JSON 文件
...
**Q: ChatWorkflowToolExecutor 如何处理复合节点？**
A: `createNodesByDefinition` 根据 `NodeTypeDefinition.compound` 配置自动生成多个子节点（含 role/rootId/parentId/scopeBoundary 等 composite 元信息）和内部边。渲染端通过 `workflow-composite.ts` 的工具函数过滤隐藏节点。

**Q: client 插件工具在 backend 如何执行？**
A: `ChatToolAdapter` 通过 `ClientNodeCache.hasClientTool()` 判断工具是否来自 client 插件。如果是，则通过 `InteractionManager` 发起 WS interaction 请求客户端执行；如果不是，直接调用 `pluginRegistry.executeAgentTool()` 在 backend 执行。
...
backend/
  ...
  chat/
    ...
    chat-event-sender.ts               Chat 事件发送器（9 事件通道）
    chat-tool-adapter.ts               Chat 工具 MCP 桥接适配器
    chat-workflow-tool-executor.ts     Chat 工作流图结构操作执行器
    client-node-cache.ts               客户端注册节点/工具缓存
  dashboard/
    stats-store.ts                     Dashboard 统计聚合与缓存
```
